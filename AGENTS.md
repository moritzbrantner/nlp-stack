...
- Keep NLP dependent only on `moenarch-foundation` crates. Normal development may use the exact foundation source revisions declared in `.coding-tooling.source-deps.json`; distribution remains registry-only.
