...
use super::*;
use crate::{
    HashedTextEmbedder, IndexBuildOptions, IndexDocument, IndexInspectReport, IndexMutationReport,
    MemoryIndexStore, StoredVector, TextIndex, TextIndexError,
};
use std::cell::Cell;
...
impl TextIndexStore for ObservedStore {
    ...
    fn upsert_document_state(
        ...
        self.inner
            .upsert_document_state(document, chunks, vectors, options)
    }
    ...
    fn vectors(&self) -> crate::Result<Vec<StoredVector>> {
        ...
        if self.unavailable {
            return Err(TextIndexError::InvalidState(
                "vector storage unavailable".to_string(),
            ));
        }
        let vectors = self.inner.vectors()?;
        self.scalars
            .set(self.scalars.get() + vectors.iter().map(|v| v.vector.len()).sum::<usize>());
        Ok(vectors)
    }
...
fn observed_index(count: usize, dimensions: usize) -> TextIndex<HashedTextEmbedder, ObservedStore> {
    ...
    index
        .upsert_documents(&documents(count))
        .expect("documents");
    index
}
...
fn lexical_performs_no_vector_reads_or_materialization() {
    ...
    assert!(!index
        .search(&IndexQuery::lexical("strategy", 3))
        .unwrap()
        .is_empty());
...
fn semantic_and_hybrid_read_and_use_vectors_once() {
    for query in [
        IndexQuery::semantic("strategy retrieval", 3),
        IndexQuery::new("strategy retrieval", 3),
    ] {
        let index = observed_index(8, 128);
        ...
        assert!(results
            .iter()
            .any(|r| r.score_breakdown.semantic_score > 0.0));
...
fn semantic_and_hybrid_propagate_vector_storage_errors() {
    for query in [
        IndexQuery::semantic("strategy", 3),
        IndexQuery::new("strategy", 3),
    ] {
        let mut index = observed_index(8, 128);
        index.store_mut().unavailable = true;
        assert!(
            matches!(index.search(&query), Err(TextIndexError::InvalidState(message))
...
fn lexical_preserves_scores_phrases_filters_and_explanations_without_vectors() {
    ...
    assert!(expected
        .iter()
        .all(|r| r.matched_phrases == ["shared evidence"]));
    index.store_mut().unavailable = true;
...
fn invalid_query_is_rejected_before_vector_storage() {
    ...
    index.store_mut().unavailable = true;
    assert!(matches!(
        index.search(&IndexQuery::lexical("strategy", 0)),
...
fn fuzzy_sqlite_variants_do_not_read_corrupt_vector_payloads() {
    let mut index =
        TextIndex::with_store(embedder(128), crate::SqliteIndexStore::in_memory().unwrap());
    ...
    assert_eq!(
        build_query_variants(&normalized_query_tokens(&query.text), &vocabulary, &options).len(),
        4
    );
    ...
    if let SurfaceIndex::Sqlite(index) = &surface {
        let changed = index
            .store()
            .connection
            .execute("UPDATE vectors SET vector_blob = X'01'", [])
            .unwrap();
        assert_eq!(changed, 8);
        assert!(
            index.store().vectors().is_err(),
            "positive control must detect corrupt vectors"
        );
    }
    assert_eq!(search(&surface, &query, &options).unwrap(), expected);
    assert!(surface
        .search(&IndexQuery::semantic("strategy", 3))
        .is_err());
...
fn query_only_benchmark() {
    ...
    for count in [32, 256] {
        for dimensions in [128, 1024] {
            ...
            let options = FuzzySearchOptions::default();
            for (kind, text) in [
                ("lexical", "strategy retrieval"),
                ("fuzzy-one", "stratgey retrieval"),
                ("fuzzy-two", "stratgey retrieavl"),
            ] {
                let query = IndexQuery::lexical(text, 8);
                let vocabulary =
                    build_vocabulary(&index_chunks(&surface).unwrap(), &options).unwrap();
                let variants =
                    build_query_variants(&normalized_query_tokens(text), &vocabulary, &options)
                        .len();
                let operation = || {
                    if kind == "lexical" {
                ...
                let work = if kind == "lexical" {
                    ...
                    assert_eq!(
                        serde_json::to_value(observed.search(&query).unwrap()).unwrap(),
                        expected
                    );
